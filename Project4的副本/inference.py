...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        return random.choices(key, weights=weights)[0]
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        tureOrFalse = (ghostPosition == jailPosition)
        true = 1.0
        if noisyDistance is None:
            return float(tureOrFalse)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        #self.numParticle
        num_p = self.numParticles
        num_l = self.legalPositions

        num_mod = num_p // len(num_l)
        num_remain = num_p % len(num_l)

        particles = num_l * num_mod
        particles += random.sample(num_l, num_remain)        
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        pos_pac = gameState.getPacmanPosition()
        pos_jail = self.getJailPosition()
        dis_belief = self.getBeliefDistribution()
 

        for particle in dis_belief:
            dis_belief[particle] *= self.getObservationProb(observation,pos_pac,particle, pos_jail)
            #When all particles receive zero weight, the list of particles should
            #be reinitialized by calling initializeUniformly.
        if dis_belief.total() == 0:
            self.initializeUniformly(gameState)
        else:
            key_num =  self.numParticles
            list_weight = list(dis_belief.values())
            
            self.particles = random.choices(list(dis_belief), k= key_num, 
                                            weights=list_weight)
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        lst_new_partic = list()
        for old_possition in self.particles:
            new_position_dis = self.getPositionDistribution(gameState, old_possition)
            newPos = new_position_dis.sample()
            lst_new_partic.append(newPos)
        self.particles = lst_new_partic
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        #distribution 
        dict = Counter(self.particles)
        distribution = DiscreteDistribution(dict)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for positions in lst_poss_ghost[1:]:
            random.shuffle(positions)
        self.particles = list(zip(*lst_poss_ghost))
